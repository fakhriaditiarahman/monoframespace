...
/* ⚡ Bolt Optimization:
 * 1. Extracted `NARRATIVE_TEXT` and `NARRATIVE_WORDS` outside the component to prevent
 *    redundant `string.split()` calculations and GC pressure on every render.
 * 2. Extracted the loop body into `NarrativeWord` to comply with React's Rules of Hooks.
 *    Calling `useTransform` inside an array `.map()` can cause hook order instability and memory leaks.
 */
